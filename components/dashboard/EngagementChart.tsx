...
const buildData = (period: "7d" | "14d" | "30d") => {
  ...
  return Array.from({ length: days }, (_, i) => {
    ...
    return {
      ...
      "DMs Sent": period === "7d" ? dms7[i]      : Math.round(100 + base * 350 + Math.random() * 40),
...
const CustomTooltip = ({ active, payload, label }: { active?: boolean; payload?: { name: string; value: number; color: string }[]; label?: string }) => {
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg p-3 text-xs" style={{ border: "0.5px solid #D9D9D9" }}>
      <p className="font-medium text-[#1A1A1A] mb-2">{label}</p>
      {payload.map((p) => (
        <div key={p.name} className="flex items-center gap-2 mb-1">
          <span className="w-2 h-2 rounded-full" style={{ background: p.color }} />
          <span className="text-[#6B7280]">{p.name}:</span>
          <span className="font-medium text-[#1A1A1A]">{p.value.toLocaleString()}</span>
        </div>
      ))}
    </div>
  );
  ...
  return (
    <div className="bg-white rounded-2xl p-6" style={{ border: "0.5px solid #D9D9D9" }}>
      ...
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-[15px] font-medium text-[#1A1A1A]">Engagement Overview</h2>
      ...
      <div className="flex gap-2 mb-5 flex-wrap">
        {(Object.keys(SERIES) as SeriesKey[]).map((key) => (
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium text-[#374151] bg-white transition-opacity ${
              hidden[key] ? "opacity-40" : "opacity-100"
            }`}
            style={{ border: "0.5px solid #D9D9D9" }}
          >
            <span className="w-2 h-2 rounded-full" style={{ background: SERIES[key] }} />
            {key}
          </button>
        ))}
      </div>
      ...
      <div className="h-[220px]">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={data} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
            ...
            <XAxis
              dataKey="label"
              tick={{ fill: "#9CA3AF", fontSize: 11, fontFamily: "Roboto" }}
              axisLine={false}
              tickLine={false}
            ...
            {(Object.entries(SERIES) as [string, string][]).map(([key, color]) => (
              <Line
                key={key}
                type="monotone"
                dataKey={key}
